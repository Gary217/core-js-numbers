 ...
 *   0    => 0
 */
const getCircleCircumference = (r) => 2 * 3.141592653589793 * r;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  // Вычисляем скалярное произведение векторов (x1, y1) и (x2, y2)
  const scal = x1 * x2 + y1 * y2;

  // Вычисляем длины (модули) векторов (x1, y1) и (x2, y2)
  const length1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const length2 = Math.sqrt(x2 ** 2 + y2 ** 2);

  // Вычисляем угол между векторами в радианах с использованием обратного косинуса
  const result = scal / (length1 * length2);
...
function toNumber(value, def) {
  ...
  let result;
  if (typeof str === 'number' && Number.isNaN(str) === false) {
    result = str;
  } else {
    result = def;
  }
...
function isSafeInteger(number) {
  return Number.isSafeInteger(number) && Number.isInteger(number);
}
